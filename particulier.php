...
<body>
    ...
    <header class="pt-40 pb-20 bg-zinc-50 border-b border-zinc-100">
        <div class="max-w-7xl mx-auto px-6 text-center">
            ...
            <h1 class="font-luxury text-4xl md:text-6xl mb-4">Construisez <span>votre Rêve</span></h1>
            <p class="text-zinc-400 max-w-2xl mx-auto text-sm">Bénéficiez de l'excellence technique de Leader Construction pour transformer votre vision en réalité, avec une garantie totale de qualité et de délais.</p>
        </div>
    </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            <div class="group bg-white border border-zinc-100 overflow-hidden hover:shadow-2xl transition-all duration-500">
                <div class="relative h-72 overflow-hidden">
                    <img src="uploads/<?php echo $b['photo_principale']; ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                    <div class="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 text-[10px] font-bold tracking-widest uppercase">
                        <?php echo htmlspecialchars($b['localisation']); ?>
                    </div>
                </div>
                <div class="p-8">
                    <h3 class="font-luxury text-2xl mb-4 italic"><span><?php echo htmlspecialchars($b['nom']); ?></span></h3>
                    ...
                    <div class="flex items-center justify-between border-y border-zinc-50 py-4 mb-6">
                        <div class="text-center">
                            ...
                            <p class="font-bold text-sm"><?php echo htmlspecialchars($b['pieces']); ?> Pièces</p>
                        </div>
                        <div class="w-px h-8 bg-zinc-100"></div>
                        <div class="text-center">
                            <p class="text-[9px] text-zinc-400 uppercase font-bold tracking-tighter">Surface</p>
                            <p class="font-bold text-sm"><?php echo htmlspecialchars($b['superficie']); ?> m²</p>
                        </div>
                        <div class="w-px h-8 bg-zinc-100"></div>
                        <div class="text-center">
                            <p class="text-[9px] text-zinc-400 uppercase font-bold tracking-tighter">Prix</p>
                           <p class="text-zinc-900 font-bold text-sm italic">
    <?php 
    if (!empty($b['prix'])) {
        // On retire les espaces ou caractères bizarres pour n'avoir que les chiffres
        $prix_nettoye = preg_replace('/[^0-9]/', '', $b['prix']); 
        echo number_format((float)$prix_nettoye, 0, ',', ' ') . ' F';
    } else {
        echo 'Sur demande';
    }
    ?>
    ...
    Découvrir la demeure →
            ...
            <?php if(empty($biens)): ?>
                <div class="col-span-full py-20 text-center">
                    <p class="text-zinc-400 italic">Aucune propriété particulière disponible pour le moment.</p>
                </div>
            <?php endif; ?>
